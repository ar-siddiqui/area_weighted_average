...
class WeightedAverage(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(QgsProcessingParameterField('AdditionalFields', 'Additional Fields to keep for HTML Table', optional=True, type=QgsProcessingParameterField.Any, parentLayerParameterName='overlaylayer', allowMultiple=True,))
        ...
        self.addParameter(QgsProcessingParameterFeatureSink('Intersected', 'Intersected', type=QgsProcessing.TypeVectorAnyGeometry, createByDefault=True, supportsAppend=True, defaultValue=None))
    ...
    def processAlgorithm(self, parameters, context, model_feedback):
        ...
        outputs['Add_id_field'] = processing.run('native:addautoincrementalfield', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        ...
        outputs['Add_area_field'] = processing.run('native:fieldcalculator', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        ...
        outputs['Add_Weight'] = processing.run('native:fieldcalculator', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        results['HTML'] = outputs['Add_Weight']['OUTPUT']   # to be commented
        ...
        alg_params = {
            ...
            'FIELD_PRECISION': 0,
            'FIELD_TYPE': 0,
            'FORMULA': ' sum("' + parameters['fieldtoaverage'] + '_Area''",\"__Unique_ID__\")/\"__Area_SPW__\"',
            'INPUT': outputs['Add_Weight']['OUTPUT'],
            'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT
        }
        outputs['area_average'] = processing.run('native:fieldcalculator', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        ...
        # Drop field(s)
        alg_params = {
            'COLUMN': ['__Unique_ID__','__Area_SPW__']+[parameters['fieldtoaverage']]+parameters['AdditionalFields']+[parameters['fieldtoaverage'] + '_Area'],
            'INPUT': outputs['Dissolve']['OUTPUT'],
            'OUTPUT': parameters['result']
        }
        outputs[result_name] = processing.run('qgis:deletecolumn', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
